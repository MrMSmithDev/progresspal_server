...
module.exports.verifyWorkoutInput = function (date, length, exercises, unit) {
  ...
  // Validate unit is of expected type
  if (unit !== "met" && unit !== "imp") {
    return {
      status: 400,
...
module.exports.verifyWeightInput = function (date, unit, weight) {
  if (!(date instanceof Date) || date > new Date()) {
    return { status: 401, error: "Invalid date" };
  }

  if (unit !== "met" && unit !== "imp") {
    return { status: 401, error: "Invalid unit selector" };
  }

  if (!/^\d+$/.test(weight) || isNaN(parseInt(weight))) {
