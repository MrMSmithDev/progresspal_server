...
function getRandomDate() {
  ...
  const pastMonth = new Date().setMonth(currentDate.getMonth() - 1);
  return faker.date.between({ from: pastMonth, to: currentDate });
}

async function generateWorkouts() {
  const workouts = [];
  for (let i = 0; i < num_workouts; i++) {
    const numExercises = faker.number.int({ min: 1, max: 3 }); // Generate 1-3 exercises

    const exercises = Array.from({ length: numExercises }, () => {
      ...
      return {
        ...
        time: type === "cardio" ? faker.number.int({ min: 1, max: 60 }) : null,
      };
    });
